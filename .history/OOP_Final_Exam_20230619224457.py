    ...
    def deposit(self, name, amount):
        if name in self.accounts:
            if amount > 0:
                ...
                print(f"BDT {amount} Deposited successfully into {name}'s account.")
            else:
        ...
        if sender_name in self.accounts and recipient_name in self.accounts:
            if amount > 0:
                if self.accounts[sender_name] >= amount:
                    ...
                    print(f"Transferred has been successfully {amount} from account {sender_name} account to {recipient_name} account.")
                    if self.total_balance < 0:
                        print("Bank is bankrupt!")
                else:
                    print("Insufficient balance in your account!")
    ...
    def process_loan(self, name):
        if name in self.accounts:
            if self.loan_enabled:
                ...
                print("Loan feature is currently disabled by bank Admin.")
        else:
            print("Account number not found!")
